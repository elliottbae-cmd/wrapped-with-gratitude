"""Home dashboard — body of the WIG Dashboard page.

Loaded by app.py via st.navigation. Page config + auth gate live in app.py.
"""
...
from ui.auth import get_client, is_admin
...
# ---- Marketing + Expenses cards ----
mr1, mr2 = st.columns(2, gap="medium")
with mr1:
    with st.container(border=True):
        st.markdown("### 📣 Marketing")
        st.caption(
            "Polish phone photos for Instagram, draft captions with AI, and run "
            "email campaigns to past customers."
        )
        st.page_link("pages/7_Marketing.py", label="Open marketing →")

with mr2:
    with st.container(border=True):
        st.markdown("### 💳 Operating expenses")
        st.caption(
            "Log software subs, packaging, postage, marketing spend. "
            "Flows into P&L net income and BS cash."
        )
        st.page_link("pages/8_Expenses.py", label="Log an expense →")
